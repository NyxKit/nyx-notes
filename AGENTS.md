...
|---|---|---|---|
| 2026-09-15 | `app/src/notes/components/NoteToolbar.vue` | Conditionally mounting `Saving…` expanded the toolbar by 26px. The initial visibility-based fix passed snapshot checks, but the user's subsequent recording still showed body text moving when the indicator appeared. | Strengthened fix: absolutely positioned indicator with a minimum-height metadata row. Frame-by-frame text-position checks pass in Chromium and Firefox; WebKit unavailable due to missing system dependencies. |
| 2026-09-15 | `AppLayout.vue`, `SettingsView.vue`, `VaultSettingsView.vue`, `TeamSettingsView.vue` | Authenticated pages requested `min-height: 100vh` below the shell header. Lazy-loaded page styles could override the shell's minimum height, making Settings extend 52px below the viewport and clipping its bottom. | Fixed: shell owns dynamic viewport sizing; conflicting page minimums removed. Navigation/reload coverage passes at 1280×720 and 800×600. |
| 2026-09-15 | `app/src/auth/composables/useAuth.ts`, note/comment ownership checks, `docs/interface/frontend.md` | `currentUser` was never populated, so authenticated authors were treated as non-authors. Ownership now uses `/api/server.current_user_id`, cleared on profile bootstrap/logout and guarded against stale responses. Editor docs now use nyx-kit's `disabled` prop. | Fixed; author typing/autosave/reload and non-author permission scenarios pass browser tests |
| 2026-09-15 | nyx-kit 2.0.22 editor dependency | Annotation context reads beyond the document end; Underline is registered both explicitly and through StarterKit. These dependency issues are separate from the missing author identity. | Open: upstream nyx-kit fixes needed |
