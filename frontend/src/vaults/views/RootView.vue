...
async function loadOverview() {
  await loadVaults()
  overviewVaults.value = props.scope === 'personal'
    ? await fetchPersonalVaults()
    : await fetchServerVaults()
  ...
  if (isInitialLoad && visibleVaults.value.length === 1) {
    router.replace(vaultRoute(visibleVaults.value[0]))
  }
}

const visibleVaults = computed(() => overviewVaults.value)
...
async function submitCreate() {
  ...
  try {
    ...
    const vault = props.scope === 'personal'
      ? await createVault(body)
      : await createServerVault(body)
    overviewVaults.value = [...overviewVaults.value, vault]
    router.push(vaultRoute(vault))
  } finally {
    ...
    <main class="home-view__body">
      ...
      <div v-else class="home-view__canvas home-view__canvas--overview">
        <NyxGrid :title="gridTitle" :mode="NyxGridMode.Grid" :columns="5">
          ...
          <NyxCard v-if="showCreateForm && canCreate" class="home-view__create-card">
