...
A Vue 3 SPA using `nyx-kit` as the component library and TipTap as the Markdown editor. Auth UI adapts to the server's `AUTH_MODE` — no login screen in `local` mode, a simple form in `secret_key` mode, redirect flow in `oidc` mode. Three-panel layout: vault/note list · editor · comment sidebar.
